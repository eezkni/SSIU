...
def _cut_out(img, n_holes, length, rate):
    b, c, h, w = img.size()
...
def cut_out(img, n_holes, length, rate=0.5):
    
    if img.size(0) > 1:
        i1, i2 = img.chunk(2, dim=0)
        
        i1 = _cut_out(i1, n_holes=n_holes, length=length, rate=rate)
        i2 = _cut_out(i2, n_holes=n_holes, length=length, rate=rate)
...
def mask_out(img, rate=0.5):
    
    B, C, H, W = img.shape
    mask = torch.rand(B, 1, H, W)
    mask = mask.ge(rate).to(img.device, dtype=img.dtype)
    img = img * mask
