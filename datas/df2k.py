...
import time
from utils import ndarray2tensor, Cutout
import cv2
...
class DF2K(data.Dataset):
    def __init__(
        ...
        if self.train:
            #DLV2K训练图片序号
            self.start_idx_div = 1
            self.end_idx_div = 801
            #Flickr训练图片序号
            self.start_idx_flv = 901
            self.end_idx_flv = 3551
        else:
            #DLV2K验证图片序号
            self.start_idx_div = 801
            self.end_idx_div = 901

        for i in range(self.start_idx_div, self.end_idx_div):
            idx = str(i).zfill(4)
        ...
        for i in range(self.start_idx_flv, self.end_idx_flv):
            idx = str(i).zfill(4)
            hr_filename = os.path.join(self.HR_folder, idx + self.img_postfix)
            lr_filename = os.path.join(self.LR_folder, 'X{}'.format(self.scale), idx + 'x{}'.format(self.scale) + self.img_postfix)
            self.hr_filenames.append(hr_filename)
        ...
        LEN = self.end_idx_div - self.start_idx_div + self.end_idx_flv - self.start_idx_flv
...
if __name__ == '__main__':
    ...
    HR_folder = 'D:\\FIV2K\\Flickr2K\\Flickr2K\\Flickr2K_HR'
    LR_folder = 'D:\\FIV2K\\Flickr2K\\Flickr2K\\Flickr2K_LR_bicubic'
    argment   = True
    df2k = DF2K(HR_folder, LR_folder, augment=True, scale=2, colors=3, patch_size=96, repeat=168, store_in_ram=True)
